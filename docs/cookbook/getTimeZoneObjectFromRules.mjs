...
class RulesTimeZone extends TimeZone {
  ...
  // {
  //   offset: number;  // UTC offset in nanoseconds
  ...
  // cache anything.
  _computeNextTransition(startingPoint, prevOffsetNs) {
    ...
    if (!rules && !until) {
      // no more transitions ever
      return { offset: stdoffNs, until: null };
    }
    ...
    if (!rules) {
      // no DST in effect
      return { offset: stdoffNs, until: stdoffZone.getInstantFor(until) };
    }
    ...
    for (const { transitionDateTime, save, timeZone } of dstRules) {
      if (PlainDateTime.compare(dateTime, transitionDateTime) < 0) {
        ...
        if (offset !== prevOffsetNs) {
          ...
          return { offset, until: transitionZone.getInstantFor(transitionDateTime) };
        }
      }
      dstShift = save;
    }
    // If we get to this point, dateTime occurs after the last DST transition in
    // that calendar year. Try again with next year's transitions.
    dateTime = dateTime.add({ years: 1 }).with({ month: 1, day: 1 });
    return this._computeNextTransition(stdoffZone.getInstantFor(dateTime), prevOffsetNs);
  }
  ...
  _computeNextTransitionCached() {
    ...
    if (this._cachedTransitions.length) {
      ...
      ({ until, offset } = lastEntry);
    }
    const result = this._computeNextTransition(until, offset);
